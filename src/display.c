/*
 *DESC: display routines to look at the tree and put something on the screen.
 */
...
int redo;		/* flag to say if we should redo */
{
	...
	if (tracing) fprintf(dtrace, "display, redo %d, big_change %d\n", redo, startpc);
#endif
	curOn( win->w_desc );

	if( redo || win->big_change ) {
#ifdef DB_DISPLAY
		if( tracing ) fprintf( dtrace, "Display all of window\n" );
#endif
#ifdef msdos
...
#ifdef DB_DISPLAY
			if(tracing)fprintf(dtrace,"Display change line %d\n", lno );
#endif
			cur_line = lno;
...
int scr_width;		/* how wide the screen */
{
	...
	main_indent = HTABSIZE * ind_level;
	if( main_indent > scr_width - HTABSIZE * 4 )
		main_indent = scr_width - HTABSIZE * 4;
	return main_indent;
}
...
int column;
{
	...
	indent = ourline.sub_line ? main_indent + HTABSIZE/2 : main_indent;
...
#ifdef DB_DISPLAY
	if(tracing) fprintf( dtrace, "find_alice_cursor:Found node at %x for row %d, column %d\n", pointed_node, row, column );
#endif
