...
/*
 *DESC: Runtime handling of the 'trace' (do..set) facility.
 *DESC: Plus turbo pascal library handling.
 */
...
traceingoff( allofthem, xlower, xupper )
int allofthem;
...
gotlib:
	...
	if( argc == 2 ) {
		...
		newstack = (unsigned int *)(frameptr) - scan_decl(
			kid2(my_block(ex_loc)),argmap,0,TRUE,FALSE,frameptr);
		}
	 else 
...
char *fname;
{
	...
	pointer arena; 
	pointer readloc;
	long flen, size_of_file();

	if( cf = qfopen( fname, "rb" ) ) {
		flen = size_of_file( cf );
