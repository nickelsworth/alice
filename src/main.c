...
int done(char *args)
{
  if (!(args && args[0]))
  {
     ...
     {
	if (!com_menu(M_quit, TRUE)) return;
     }
  }
...
int escmaps[] = { '\n', AL_ICOMMAND, KEY_BACKSPACE, KEY_UP, KEY_DOWN,
		  KEY_LEFT, KEY_RIGHT, 4 /*ctrl d */, AL_ID, '\\',
		  AL_FNAME };

void macroize(char *str)
{
...
void sigoccurred(int sn)
{
   if (++bugcount > 20) done((char *)0);
   catchsigs();
   if (sn == SIGINT) {
      didbreak = TRUE;
      bugcount = 0;
   }
   else error(ER(70,"Internal error: UNIX signal %d"), sn);
}

static int gotdeath = 0;

static void abfush(int sn)
{
   if( gotdeath > 2 ) abort();
   ++gotdeath;
#ifdef DEBUG
   if( dtrace ) fflush(dtrace);
...
int do_cloption(char *argline, int mustmalloc) /* argline stuff will go away */
{
   ...
   if( !argline ) error( ER(71,opterstr), "<null>" );
	 ...
	 case 'm':
	    if( isdigit( argline[1] ) )
	       macnum = strtol( argline+1, NULL,
				argline[1] == '0' ? 8 : 10 );
	    else if( argline[1] == '\'' ) macnum = argline[2];
	    else if( argline[1] == '^' ) macnum = argline[2] & 0x1f;
	    else error( ER(72,"opt`Invalid m#= option") );
	    if (macnum >= 0 && macnum < HIGHKEY) {
		  ...
		  argstr;
	       maccount++;
	    } else error(ER(73,"opt`macro %d out of range"), macnum);
	    break;
	 case 'i': /* init string */
...

	 ...
	 case 'u':       /* unbuffered trace file */
	    if (*argstr) strcpy(trace_name, argstr);
	    else sprintf(trace_name, "atr%d", getuid());
	    if (c != 'f') dtrace = fopen(trace_name, "w");
	    if (c == 'u') setbuf( dtrace, (pointer)NULL );
	    break;
#endif DEBUG
	 default: error( ER(71,opterstr), argline );
      }
  }
...

   ...
   else LoadFile = allocstring(argline);
}


void gotHere(char *file, int line)
{
   if (tracing) fprintf(dtrace, "[At line %d in %s]\n", line, file);
}
