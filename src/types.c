/*
 *DESC: Routines to do type comparisons, checks and manipulations
 */
...
#ifdef DB_TYPES
	if(tracing)fprintf(dtrace, "Look for type tree of %x\n", decl );
#endif
...
#ifdef DB_TYPES
	if(tracing)fprintf(dtrace, "Find_realtype returns %x from %x\n", f_real_node, xstart );
#endif
	return f_real_node;
}
...
{
	register nodep tree;
	unsigned stemp;		/* temporary in getting compound type size */
	...
	switch( ntype(tree) ) {
...
#ifdef DB_TYPES
			if(tracing)fprintf(dtrace,"Calc-size of id %x\n", tree);
#endif
		...
		case N_TYP_ARRAY:
			...
			if( !runtime ) {
				...
				s_int_kid(4, tree, range_check( kid1(tree), 
					&lowbound ));
				stemp = calc_size( component, runtime );
				s_int_kid( 2, tree, stemp );
					/* set multiplier */
				if( lowbound == 1 && ntype(tparent(tree)) ==
					N_TYP_PACKED && ntype(component) == N_ID
					&&kid_id(component)==(symptr)Basetype(BT_char)){
					/* BUG, should check for integer subrange type */
				...
				stemp = int_kid( 2, tree );
				isstr = (node_flag(tree) & NF_STRING) != 0;
				}
			/* add one to size of array of bytes for zero in str */
			/* also add one for size byte */

			arsize = isstr + stemp * int_kid(4,tree);

			if( arsize < 0 ) {
				type_error( tree, ER(277,"bigstruct`Array much too large -- shrink the bounds") );
				arsize = 2;
				}
			return arsize;
			}
		...
		case N_TYP_FILE:
			...
			return sizeof(struct pas_file) - sizeof(char) +
				calc_size( comtyp, runtime );
			}
		case N_TYP_RECORD:
			if( !runtime && kid1(tree) )
				s_int_kid( 2, tree,
					scan_decl( FLCAST kid1(tree), fd_func, 0, FALSE, NF_ERROR, (pint)0) );
			return int_kid(2, tree );
		case N_STUB:
...
#endif
#ifdef DB_TYPES
	if(tracing)fprintf(dtrace,"Evaluate constant at %x\n", contree );
#endif
...
nodep xtype_parent;	/* the N_FIELD node */
{
	...
	if( not_a_stub( thedecl ) ) {
		...
		so_far += howbig;
		if( so_far < 0 ) {
			type_error( type_parent, ER(277,"bigstruct`Structure larger than maximum size -- shrink it.") );
			so_far -= howbig;
			}
		}
	...
	for( i = 0; i < listcount(varlist) ; i++ ) {
		...
		/* what about overflow? */
		nsf = max(nsf, scan_decl( FLCAST kid2(varnode), fd_func,
				so_far, FALSE, NF_ERROR, (pint)0 ) );
